...
describe("First-Principles CDI Model Unit & Validation Suite", () => {
    ...
    it("verifies first-principles reference benchmark case for single-stage CDI", () => {
        ...
        expect(res.totalFaradayCurrent).toBeCloseTo(151.75, 1);
        expect(res.cellPairs).toBe(102);
        expect(res.cellCurrent).toBeCloseTo(1.49, 2);
        expect(res.voltageStack).toBe(122.40);
        expect(res.power).toBeCloseTo(182.0, 0);
        expect(res.secElectrical).toBeCloseTo(0.379, 2);
        expect(res.envelopeStatus).toBe("MODEL_PREDICTION");
    });

    it("verifies operating envelope status categorization", () => {
        // Model prediction case (< 1000 ppm, <= 1.2V, <= 75% removal)
