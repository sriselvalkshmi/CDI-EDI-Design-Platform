...
export function validateFormula(formulaStr) {
    ...
    try {
        ...
        // Scope with Math context available
        const testFn = new Function("Math", ...keys, `with(Math) { return ${jsExpression}; }`);
        testFn(Math, ...values);
...
export function evaluateFormula(formulaStr, scope = {}) {
    ...
    try {
        ...
        const fn = new Function("Math", ...keys, `with(Math) { return ${jsExpression}; }`);
        const res = fn(Math, ...values);
