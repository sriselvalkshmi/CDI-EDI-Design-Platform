...
import { DEFAULT_EQUATIONS_DATABASE } from '../equations/defaultEquationsDatabase.js';
...
class EquationEngine {
    ...
    static setSupabaseClient(client) {
    ...
    static async loadEquationsAsync() {
        if (supabaseClientInstance) {
            try {
                const { data, error } = await supabaseClientInstance.from('equations').select('*');
