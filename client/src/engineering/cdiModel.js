...
export const CDI_ENVELOPE = {
    ...
    calibrationStatus: "Validated against peer-reviewed literature (Porada et al., 2013; Biesheuvel et al., 2011)"
};
...
export function calculateCDIChargeEfficiency(cellVoltage = 1.2, feedTds = 500, customConfig = {}) {
    ...
    // Baseline nominal efficiency for standard porous carbon at 1.2V and 500 ppm is 0.82 (82%)
    const baseLambda = 0.82;
    const voltageFactor = 1.0 - 0.12 * ((cellVoltage - 1.2) / 1.2);
    ...
    return Math.max(0.40, Math.min(0.85, Number(lambda.toFixed(4))));
}
...
export function calculateCDIModel(inputs = {}) {
    ...
    // 2. Single-Stage Removal Limit & Mass Balance
    // Uncoated CDI maximum single-pass removal is 75% (Porada et al., 2013)
    const requestedRemovalRatio = feedTds > 0 ? Math.max(0.05, (feedTds - targetTds) / feedTds) : 0.75;
    const maxSingleStageRemoval = CDI_ENVELOPE.maxSingleStageRemovalRatio;
    const actualRemovalRatio = Math.min(maxSingleStageRemoval, requestedRemovalRatio);
    ...
    const waterRecoveryPct = Number(inputs.waterRecovery ?? CDI_ENVELOPE.defaultRecovery); // %
    ...
    // 10. Target Feasibility & Technology Envelope Status Evaluation
    const isTargetAchieved = outletTds <= targetTds + 0.5;

    let envelopeStatus = "VALIDATED";
    let envelopeMessage = "Operating parameters within validated literature benchmark envelope (Porada et al., 2013).";

    if (requestedRemovalRatio > maxSingleStageRemoval) {
        envelopeStatus = "EXTRAPOLATED";
        envelopeMessage = `MODEL OPERATING LIMIT: Under current CDI model assumptions and validated operating envelope, the predicted single-stage outlet is ${outletTds} mg/L. Additional polishing or MCDI is required for the ${targetTds} mg/L setpoint.`;
    }
    ...
    if (feedTds > CDI_ENVELOPE.maxValidatedTds || cellVoltage > CDI_ENVELOPE.maxCellVoltage) {
        envelopeStatus = "OUTSIDE_ENVELOPE";
        envelopeMessage = `Operating point exceeds validated CDI feasibility boundaries (> 3,000 mg/L TDS or > 1.5 V). MCDI or FCDI is strongly recommended.`;
    }
    ...
    return {
        ...
        // Mass Balance Conservation Audit Flags
        isWaterConserved,
        isSaltConserved,
