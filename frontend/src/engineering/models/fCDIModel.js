...
export const FCDI_ENVELOPE = {
    ...
    recommendedTdsRange: { min: 1000, max: 15000 }, // mg/L (Current Model Envelope)
    maxLiteratureCapabilityTds: 50000, // mg/L (Literature Demonstrated Capability benchmark: Jeon et al. 2013)
    ...
    provenance: {
        ...
        maxLiteratureCapabilityTds: "LITERATURE_SUPPORTED (Jeon et al., 2013; Rommerskirchen et al., 2018)",
    ...
    calibrationStatus: "Engineering model internally verified against stated assumptions and literature benchmarks (Jeon et al., 2013; Rommerskirchen et al., 2018)"
};
...
export function calculateFCDIModel(inputs = {}) {
    ...
    if (feedTds > FCDI_ENVELOPE.maxLiteratureCapabilityTds || cellVoltage > FCDI_ENVELOPE.maxCellVoltage) {
        ...
        envelopeMessage = `Operating point exceeds literature capability boundaries (> ${FCDI_ENVELOPE.maxLiteratureCapabilityTds} mg/L TDS or > ${FCDI_ENVELOPE.maxCellVoltage} V).`;
    }
    ...
    const saltBalanceError = Number(Math.abs(feedSaltMassGs - (productSaltMassGs + concentrateSaltMassGs)).toFixed(4));
    ...
    const risks = [];
    if (isTargetAchieved) {
        risks.push({ level: "PASS", message: "Continuous desalting achievable under flow-electrode carbon slurry model." });
    } else {
        risks.push({ level: "FAIL", message: `Single-stage target not fully met: Outlet is ${outletTds} mg/L (Target: ${targetTds} mg/L).` });
    }

    risks.push({ level: "WARNING", message: `Slurry Pumping SEC Penalty: Slurry pump duty (${secSlurryPump} kWh/m³) represents major energy consumption due to non-Newtonian slurry viscosity.` });
    risks.push({ level: "WARNING", message: "Carbon Slurry Aggregation & Settling Risk: Requires continuous agitation and flow rate monitoring to prevent channel clogging." });
    risks.push({ level: "INFO", message: "Continuous Operation: Uninterrupted desalinated water production without cyclic batch pauses." });

    // Envelope Distinction
    const envelopeDistinction = {
        currentModelEnvelope: `1,000 – 15,000 mg/L TDS (Sized for current ${feedTds} mg/L feed)`,
        literatureEvidence: "Continuous elevated-salinity desalination and brine concentration demonstrated up to 50,000+ mg/L (arXiv:2204.08897; Rommerskirchen et al. 2018)",
        extrapolationWarning: feedTds > 15000 ? `Feed TDS (${feedTds} mg/L) is extrapolated into high-salinity brine concentration regime.` : null
    };
    ...
    return {
        ...
        calculationTrace: [
            ...
            { step: 2, name: "Feasibility Gating", status: "PASSED", provenance: "LITERATURE_SUPPORTED", detail: "High-salinity continuous flow feasible (Envelope: 1000–15000 mg/L TDS)" },
            { step: 3, name: "Physical Mechanism", status: "VALIDATED", provenance: "LITERATURE_SUPPORTED", detail: "Continuous circulating carbon slurry electrodes with ion-exchange membranes" },
            ...
            { step: 6, name: "Hydraulic Separation", status: "VALIDATED", provenance: "CALIBRATED", detail: `Separate water pump (${waterPumpPowerW} W) and non-Newtonian carbon slurry pump (${slurryPumpPowerW} W)` },
            { step: 7, name: "Outlet TDS Derivation", status: "DERIVED", provenance: "FIRST_PRINCIPLES", detail: `C_out (${outletTds} mg/L) derived from continuous flow-electrode Faraday mass transfer` },
            { step: 8, name: "Water Recovery", status: "DERIVED", provenance: "CALIBRATED", detail: `Recovery (${waterRecoveryPct}%) derived from continuous carbon slurry regeneration bleed ratio` },
            ...
            { step: 10, name: "Target Check", status: isTargetAchieved ? "PASSED" : "LIMIT_REACHED", provenance: "PROJECT_ASSUMPTION", detail: isTargetAchieved ? `Target ${targetTds} mg/L Achieved` : `Single-stage target deviation (+${(outletTds - targetTds).toFixed(1)} mg/L)` }
        ]
    };
}
